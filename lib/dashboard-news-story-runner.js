...
function renderRelatedSectors(sectors) {
  ...
  const rounded = sectors.map((s) =>
    s.pct != null ? Math.round(s.pct * 10) / 10 : null
  );
  const uniquePcts = new Set(rounded.filter((p) => p != null));
  const showAllPcts = uniquePcts.size > 1;

  return sectors
    .map((s, i) => {
      const pct = rounded[i];
      const pctHtml =
        showAllPcts && pct != null
          ? `<span class="news-live-sector__pct">${formatPct(pct)}</span>`
          : "";
      return `<li class="news-live-sector">${escapeHtml(s.label)}${pctHtml}</li>`;
    })
    .join("");
}
