...
const EXPLORE_SELECTOR =
  ".mobile-explore-dashboard, [data-split-action='demo'], .cw-hero-cta-btn[data-split-action='dashboard']";

const APP_UI_SELECTOR =
  "#page-dashboard, #page-why, #page-earnings, #page-portfolio, #page-scanner, #page-insiders, #page-logic, " +
  "#panelWheel, #dashWheelExperience, .intel-wheel__viewport, .intel-wheel__chip, .intel-wheel__track, " +
  ".briefing-rail, .briefing-rail__wheel, .briefing-main, .briefing-stage, .wheel-module-viewport, " +
  "#whmWheelViewport, #whmWheelEngine, #wheelViewport, #wheelModuleStage";
...
function isAppUiTarget(target) {
  return !!(target && typeof target.closest === "function" && target.closest(APP_UI_SELECTOR));
}
...
export function initMobileCtaHandlers() {
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      if (isAppUiTarget(e.target)) return;
