...
export async function fetchWhatMattersFeed(opts = {}) {
  ...
  const calendarCards = rankCards([...fred, ...earnings]);
  /** @type {WhatMattersCard[]} */
  const deduped = [];
  const seen = new Set();
  const seenFredEvents = new Set();

  for (const card of calendarCards) {
    if (card.source === "FRED" && seenFredEvents.has(card.event)) continue;
    const key = `${card.source}:${card.dateIso}:${card.event}`;
    if (seen.has(key)) continue;
    seen.add(key);
    if (card.source === "FRED") seenFredEvents.add(card.event);
    deduped.push(card);
    if (deduped.length >= limit) break;
  }

  if (deduped.length < limit && deduped.length < 3) {
    const newsOnly = rankCards(
      news.filter((c) => c.type === "Central Bank" || c.type === "Macro Data")
    );
    for (const card of newsOnly) {
      const key = `${card.source}:${card.dateIso}:${card.event}`;
      if (seen.has(key)) continue;
      seen.add(key);
      deduped.push(card);
