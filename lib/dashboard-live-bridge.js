...
function hydrateMoversModule(root, quotes) {
  ...
  if (active) {
    ...
    const heroPrice = root.querySelector(".movers-intel__hero-meta");
    if (heroChg && q) {
      const pct = q.pctChange ?? 0;
      const dir = pct > 0.05 ? "up" : pct < -0.05 ? "dn" : "flat";
 ...
 * @param {number} vix
 */
function hydrateVolatilityModule(root, vix) {
  const gauge = root.querySelector(".live-gauge");
  if (!gauge || vix == null || Number.isNaN(vix)) return;

  const mood = moodZoneFromVix(vix);
  const riskPct = vixToRiskScorePercent(vix);
  const geom = gaugeArcGeometry(vix);

  gauge.dataset.vix = String(vix.toFixed(1));
  gauge.dataset.mood = mood.id;
  ...
  if (summary) summary.textContent = mood.summary || mood.probe;
  const plain = root.querySelector("[data-mood-plain]");
  if (plain) plain.textContent = mood.plainEnglish;

  applyBadge(ensureModuleBadge(root), "delayed", `VIX ${vix.toFixed(1)} · FRED prior close`);
}
...
async function fetchFredVix() {
  try {
    const res = await fetch("/api/proxy?provider=fred&series=VIXCLS");
...
export async function hydrateDashboardModule(moduleId, root) {
  ...
  try {
    if (moduleId === "movers") {
      ...
      if (vix != null) hydrateVolatilityModule(root, vix);
...
/** Update dashboard pulse strip from index quotes when available. */
export async function hydrateDashboardPulse() {
  const strip = document.querySelector("#page-dashboard.active .wheel-pulse-strip");
  const fetchQuotes = getFetchQuotes();
  if (!strip || !fetchQuotes) return;
  try {
    const q = await fetchQuotes(["SPY", "QQQ"]);
    if (!q.SPY || q.SPY.pctChange == null || Number.isNaN(q.SPY.pctChange)) return;
    const spy = q.SPY?.pctChange;
    const qqq = q.QQQ?.pctChange;
    const headline = strip.querySelector(".wheel-pulse-strip__headline");
    if (headline && spy != null) {
      headline.textContent = `Session · SPY ${fmtPct(spy)}%${qqq != null ? ` · QQQ ${fmtPct(qqq)}%` : ""}`;
    }
    const badge = document.getElementById("dashLiveBadge");
    if (badge) applyBadge(badge, "mixed", "Live index quotes · narrative illustrative");
  } catch (_) {}
}
