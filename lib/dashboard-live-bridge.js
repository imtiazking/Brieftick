...
import { hydrateWhatMatters } from "/lib/what-matters-ui.js";
  ...
  try {
    if (moduleId === "movers" && fetchQuotes) {
      ...
      if (typeof window.refreshNewsStoryState === "function") {
        const snap = await window.refreshNewsStoryState({
          refreshImpact: false,
          fetchOil: true,
        });
      ...
      applyBadge(
        ensureModuleBadge(root),
        window.dashboardNewsSnapshot?.stories?.[0]?.live?.dataQuality === "live"
          ? "live"
          : "mixed",
        "Story intelligence from live market data"
      );
    } else if (moduleId === "alerts") {
      await hydrateWhatMatters(root);
    }
  } catch (e) {
...
export function startDashboardLiveRefresh() {
  ...
  const tick = () => {
    ...
    if (module.classList.contains("rail-module--intel")) hydrateDashboardModule("news", module);
    if (module.classList.contains("rail-module--what-matters")) hydrateDashboardModule("alerts", module);
  };
