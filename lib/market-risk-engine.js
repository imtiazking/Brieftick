...
function buildStructuredNarrative(p) {
  ...
  const explanation = `<b>FORGENIQ read:</b> ${p.label}. ${strongest.name} is the strongest caution signal (${Math.round(strongest.value)}/100: ${strongest.detail}), while ${calmest.name} is the least stressed (${Math.round(calmest.value)}/100). Live context score — not a forecast.`;
