...
export async function fetchPortfolioQuote(symbol, opts = {}) {
  ...
  const QR = typeof window !== "undefined" ? window.BrieftickQuoteRouter : null;
  if (QR) {
    const raw = await QR.getQuote(sym);
    if (raw?.price) {
      const quote = toPortfolioQuote(
        sym,
        { price: raw.price, change: raw.change, pctChange: raw.pctChange, name: raw.name },
        raw._provider || "quoteRouter"
      );
      if (quote) {
        cacheSetQuote(sym, quote);
        const attempts = [{ provider: quote.provider, ok: true, price: quote.price, pctChange: quote.pctChange }];
        const result = { symbol: sym, ok: true, quote, attempts };
