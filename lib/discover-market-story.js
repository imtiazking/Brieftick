...
/** @type {boolean} */
let themePricesHydrated = false;

async function hydrateThemePrices() {
  if (themePricesHydrated) return;
  const QR = window.BrieftickQuoteRouter;
  if (!QR) return;
  const syms = new Set();
  for (const theme of Object.values(THEMES)) {
    for (const s of theme.stocks || []) syms.add(s.sym);
  }
  if (!syms.size) return;
  try {
    const quotes = await QR.getQuotes([...syms]);
    for (const theme of Object.values(THEMES)) {
      for (const s of theme.stocks || []) {
        const q = quotes[s.sym];
        if (q?.price) {
          s.close = q.price;
          s.pctChange = q.pctChange;
        }
      }
    }
    themePricesHydrated = true;
  } catch (e) {
    console.warn("[discover] theme price hydration failed:", e.message);
  }
}
  ...
  return `
    <article class="discover-story-card${isExpanded ? " is-expanded" : ""}" data-theme="${theme.id}" id="discover-story-${theme.id}">
      ...
      <div class="discover-story-card__qa">
        <div>
          <p class="discover-story-card__q">What's happening</p>
          <p class="discover-story-card__a">${esc(n?.what)}</p>
        </div>
        <div>
          <p class="discover-story-card__q">Why it matters</p>
          <p class="discover-story-card__a">${esc(n?.why)}</p>
        </div>
        <div>
          <p class="discover-story-card__q">Story strength</p>
        ...
        <div>
          <p class="discover-story-card__q">What could change it</p>
          <p class="discover-story-card__a">${esc(n?.watch)}</p>
        </div>
        <div>
          <p class="discover-story-card__q">Exposed names</p>
