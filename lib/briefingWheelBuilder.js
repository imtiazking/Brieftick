...
const TWELVE_DATA_FALLBACK_MAX = 8;
 ...
 * @param {string} sym
 */
async function fetchTwelveDataQuote(sym) {
  try {
    const d = await fetchJson(
      `/api/proxy?provider=twelvedata&endpoint=quote&symbol=${encodeURIComponent(sym)}`
    );
    if (!d || d.status === "error" || d.code) return null;
    const price = parseFloat(d.close);
    const pctChange = parseFloat(d.percent_change);
...
async function fetchLiveQuotes(symbols) {
  ...
  const missing = list.filter((s) => !quotes[s]?.price).slice(0, TWELVE_DATA_FALLBACK_MAX);
  for (const sym of missing) {
    const q = await fetchTwelveDataQuote(sym);
    if (q) quotes[sym] = q;
