...
function bindPortfolioInsights(root) {
  ...
  const handoffCount = root.querySelector("#piHandoffCount");
  ...
  function openHandoff(label, tickers) {
    ...
    if (handoffBody) {
      handoffBody.textContent =
        "When you're ready, continue with Trading212 using the companies you selected. Brieftick explains — your broker executes.";
    }
    if (handoffTickers) {
    ...
    if (handoffCount) {
      handoffCount.textContent = list.length ? stockCountLabel(list.length) : "";
      handoffCount.hidden = list.length === 0;
    }
  ...
  root.addEventListener("click", (e) => {
    ...
    if (e.target.closest("[data-open-in-broker]")) {
      showToast("Prototype only — broker handoff simulated. No orders placed.");
      closeHandoff();
    }
  });
