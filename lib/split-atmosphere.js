  ...
  if (track?.innerHTML?.trim()) {
    ...
    if (items.length) {
      const parts = [...items].map((el) => {
        ...
        const rest = spans[1]?.textContent?.trim() || "";
        const chg = spans[2]?.textContent?.trim() || "";
        return `${sym} ${rest} ${chg}`.trim();
      });
      const line = parts.filter(Boolean).join(" · ") + " · BRIEFTICK · ";
      return line.repeat(12);
    }
  }
  return "SPY · QQQ · NVDA · TSLA · VIX · AAPL · AMD · META · ".repeat(12);
}
