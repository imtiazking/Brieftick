 ...
 * @returns {string}
 */
export function formatFredVixAsOf(iso) {
  if (!iso) return "As of —";
  try {
...
export function hydrateMarketRiskGauge(root, riskState) {
  ...
  if (riskState.score != null && !Number.isNaN(riskState.score)) {
    gauge.dataset.compositeScore = String(Math.round(riskState.score));
  }
  gauge.style.setProperty("--gauge-fill", String(riskPct));
  const compositeRounded =
    riskState.score != null && !Number.isNaN(riskState.score)
      ? Math.round(riskState.score)
      : Math.round(riskPct);
  gauge.setAttribute("aria-valuenow", String(compositeRounded));
  gauge.setAttribute(
    "aria-label",
    `Market risk gauge · composite score ${compositeRounded} · ${riskState.label}`
  );
  ...
  const regimeLabel = gauge.querySelector("[data-mood-label-regime]");
  if (regimeLabel) regimeLabel.textContent = riskState.label;

  const moodZone = gauge.querySelector("[data-mood-zone]");
  if (moodZone) moodZone.textContent = "Regime · needle reflects composite score";

  const compositeText =
    riskState.score != null && !Number.isNaN(riskState.score)
      ? String(Math.round(riskState.score))
      : "—";
  gauge.querySelectorAll(".gauge-composite-value, [data-composite-score]").forEach((el) => {
    if (el.classList.contains("live-gauge")) return;
    el.textContent = compositeText;
  });

  const vixEl = gauge.querySelector(".gauge-vix-value");
  if (vixEl && riskState.vix != null) {
    vixEl.textContent = riskState.vix.toFixed(1);
  }

  const vixAsOfEl = gauge.querySelector("[data-vix-asof]");
  if (vixAsOfEl) {
    const fredDate =
      typeof window !== "undefined" ? window._vixFredDate : null;
    vixAsOfEl.textContent = formatFredVixAsOf(fredDate);
  }
  ...
  if (badge) {
    const scoreTxt =
      riskState.score != null ? `Score ${Math.round(riskState.score)}/100` : "Composite risk";
    const vixTxt =
      riskState.vix != null ? ` · VIX ${riskState.vix.toFixed(1)}` : "";
    applyBadge(badge, "mixed", `${riskState.label} · ${scoreTxt}${vixTxt}`);
  }
}
