  ...
  root.innerHTML = `
    <div class="mp-root" data-mp-root hidden>
      ...
      <button type="button" class="mp-promo" data-mp-promo hidden>
        <span>New: Mission Path — Learn the workflow</span>
...
export function renderWelcome(el, progress) {
  el.innerHTML = `
    <div class="mp-welcome__inner">
      ...
      <div class="mp-block mp-block--why">
        <h3 class="mp-block__label">Why this matters</h3>
        <p class="mp-block__text">${WELCOME.whyMatters}</p>
      </div>
      <p class="mp-welcome__meta">${formatProgressMeta(0)}</p>
...
export function renderMissionPanel(el, progress, isTerminal) {
  ...
  const whenHtml = mission.whenToUse.map((w) => `<li>${w}</li>`).join("");
  ...
  if (mission.checklist?.length) {
    const items = mission.checklist
      ...
      .join("");
    taskHtml = `<div class="mp-checklist">${items}</div>`;
  } else {
  ...
  el.innerHTML = `
    ...
    <header class="mp-panel__head">
      <div>
        <p class="mp-tag">${WELCOME.tag}</p>
        <p class="mp-panel__meta">${formatProgressMeta(idx)}</p>
      </div>
    ...
    <div class="mp-panel__body">
      <h2 class="mp-panel__title">${mission.title}</h2>
      <div class="mp-block">
        <h3 class="mp-block__label">What is it?</h3>
        <p class="mp-block__text">${mission.whatIsIt}</p>
      </div>
      <div class="mp-block mp-block--why">
        <h3 class="mp-block__label">Why this matters</h3>
        <p class="mp-block__text">${mission.whyMatters}</p>
      </div>
      <div class="mp-block">
        <h3 class="mp-block__label">When to use it</h3>
        <ul class="mp-when">${whenHtml}</ul>
      </div>
      <div class="mp-block">
        <h3 class="mp-block__label">Your task</h3>
        ${taskHtml}
...
export function renderCompletion(el, progress) {
  const xp = computeXp(progress);
  const loop = COMPLETION.loop
    .map(
      (row) =>
        `<li><span class="mp-loop__n">${row.n}</span><span class="mp-loop__t">${row.title}</span><span class="mp-loop__d">→ ${row.dest}</span></li>`
    )
    .join("");
  ...
  el.innerHTML = `
    <div class="mp-complete__inner">
      ...
      <div class="mp-block">
        <h3 class="mp-block__label">Your daily loop</h3>
        <ol class="mp-loop">${loop}</ol>
      </div>
      <p class="mp-complete__xp">Total earned: ${xp} XP · ${FINAL_RANK} rank</p>
      <div class="mp-block mp-block--why">
        <h3 class="mp-block__label">Why this matters</h3>
        <p class="mp-block__text">${COMPLETION.whyMatters}</p>
      </div>
...
export function updatePill(pill, progress) {
  ...
  if (text) text.textContent = `Mission ${n} of ${TOTAL_MISSIONS} · Resume`;
}
...
export function renderSettingsStatus(statusEl, progress) {
  ...
  if (progress.status === "completed") {
    const date = progress.completedAt
      ...
      : "";
    statusEl.textContent = `Completed${date ? ` · ${date}` : ""}`;
    return;
  }
  const n = (progress.currentMission ?? 0) + 1;
  const rank = currentRankLabel(progress);
  const xp = computeXp(progress);
  statusEl.textContent = `Mission ${n} of ${TOTAL_MISSIONS} · ${rank} · ${xp} XP earned`;
}
