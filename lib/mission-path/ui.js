...
export function renderWelcome(el, progress) {
  el.innerHTML = `
    <div class="mp-welcome__inner">
      <div class="fq2-logo-stacked fq2-logo-stacked--standalone-sm mp-welcome__logo" role="img" aria-label="FORGENIQ">
        <span class="fq2-logo-stacked__symbol">
          <img src="/brand/forgeniq-symbol-white.png" alt="" width="474" height="474" decoding="async">
        </span>
        <span class="fq2-logo-stacked__wordmark fq2-wordmark-mount"></span>
      </div>
...
export function renderMissionPanel(el, progress, isTerminal) {
  ...
  el.innerHTML = `
    ...
    <div class="mp-panel__body">
      ...
      ${
        mission.route
          ? `<button type="button" class="mp-btn mp-btn--ghost mp-btn--block" data-mp-goto>Open ${mission.title === "Investigate a Move" ? "What's Moving" : mission.title === "Find Opportunities" ? "Discover" : mission.title}</button>`
          : ""
      }
    </div>
...
export function renderCompletion(el, progress) {
  ...
  el.innerHTML = `
    <div class="mp-complete__inner">
      ...
      <div class="mp-complete__next">
        <p class="mp-complete__next-label">${COMPLETION.nextStepLabel}</p>
        <p class="mp-complete__next-body">${COMPLETION.nextStepBody}</p>
      </div>
