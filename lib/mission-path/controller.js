  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      if (t.hasAttribute("data-mp-pill")) {
        progress = persist({ status: "active" });
        showMission();
      }
...
function beginMissionArc(fromWelcome = true) {
  ...
  if (fromWelcome || !progress) {
    ...
    progress = persist({ status: "active", dismissedAt: undefined, sessionStarted: true });
  }
  showMission();
}
...
export function onAuthReady(user) {
  ...
  setTimeout(() => {
    ...
    if (progress.status === "active") {
      if (progress.completedMissions?.length) showMission();
      else showWelcome();
    }
  }, 850);
}
