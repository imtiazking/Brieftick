...
export const WELCOME = {
  tag: "Session Quest",
  headline: "Learn the FORGENIQ workflow",
...
export const PROMO = {
  label: "New: Mission Path",
  detail: "Learn the workflow",
};

export const COMPLETION = {
  headline: "Mission Complete",
  subhead: "You now understand how FORGENIQ works.",
  nextStepLabel: "Next step:",
  nextStepBody: "Build your first investment basket.",
  enterCta: "Open Portfolio Insights",
...
export const MISSIONS = [
  ...
  },
  {
    ...
    xp: 50,
    route: "logic",
    lead: "Logic answers in plain English. Ask one clear question and get context: drivers, signals, sources. No jargon.",
    context:
  ...
  },
  {
    ...
    xp: 50,
    route: "scanner",
    lead: "Discover starts with market stories, not tickers. Each narrative explains what's moving, why it matters, and which names are exposed.",
    context:
      "Come here when you want fresh ideas, when a theme is heating up on the Dashboard, or when you're building a list to follow.",
    action: "Open one market story, read what's happening, then tap a stock exposed to that narrative.",
  ...
  },
  {
    ...
    xp: 75,
    route: "earnings",
    lead: "Earnings is your weekly calendar in plain English. See who reports, why it matters, and what could move the stock.",
    context:
 ...
 * @returns {{ step: string, title: string, time: string }}
 */
export function progressHead(missionIndex) {
  const idx = Math.min(Math.max(missionIndex, 0), TOTAL_MISSIONS - 1);
  const n = idx + 1;
  const mins = TIME_REMAINING[idx] ?? 1;
  const mission = MISSIONS[idx];
  return {
    step: `Mission ${n} of ${TOTAL_MISSIONS}`,
    title: mission?.title ?? "",
    time: `~${mins} min remaining`,
  };
}
...
export function formatProgressMeta(missionIndex) {
  const { step, time } = progressHead(missionIndex);
  return `${step}. ${time}`;
}
