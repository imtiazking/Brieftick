...
/** @typedef {'not_started' | 'active' | 'skipped' | 'completed' | 'dismissed'} MissionStatus */
...
/**
 * @param {MissionStatus|undefined} status
...
/**
 * @param {UserProgress|null|undefined} raw
...
export function loadProgress(userId) {
  ...
  const root = readRoot();
  return normalizeProgress(root.users[userId] ?? null);
}
...
export function markLegacyMigrated(userId) {
  ...
  saveProgress(userId, {
    ...
    status: p?.status === "completed" ? "completed" : "skipped",
    legacyMigrated: true,
    skippedAt: p?.skippedAt ?? p?.dismissedAt ?? new Date().toISOString(),
  });
}
