...
function tripTwelveDataCircuit(data) {
  if (data?.disabled) {
    ...
    logOnce("td-disabled", "Twelve Data disabled — no fallback requests");
    return;
  }
...
async function fetchFinnhubQuote(symbol) {
  if (Date.now() < finnhubOpenUntil) return null;
  const r = await fetchJson(
    `/api/proxy?provider=finnhub&endpoint=quote&symbol=${encodeURIComponent(symbol)}`
  );
  if (r.data?.circuitOpen && r.data?.rateLimited) {
    finnhubOpenUntil = Date.now() + FINNHUB_CIRCUIT_MS;
    return null;
  }
  if (r.status === 429) {
    finnhubOpenUntil = Date.now() + FINNHUB_CIRCUIT_MS;
...
async function fetchTwelveDataQuote(symbol) {
  if (twelveDataDisabled || twelveDataQuotaExhausted || Date.now() < twelveDataOpenUntil) return null;
  const r = await fetchJson(
    `/api/proxy?provider=twelvedata&endpoint=quote&symbol=${encodeURIComponent(symbol)}`
  );
  if (r.data?.disabled || r.data?.circuitOpen || r.data?.unavailable) {
    tripTwelveDataCircuit(r.data);
    return null;
  }
  if (r.status === 503 && r.data?.provider === "twelvedata") {
    tripTwelveDataCircuit(r.data);
    return null;
  }
...
async function resolveQuote(symbol) {
  ...
  return dedupe(ck, async () => {
    ...
    if (!twelveDataDisabled && !twelveDataQuotaExhausted && Date.now() >= twelveDataOpenUntil) {
      try {
...
export function getRouterStats() {
  return {
    twelveDataCircuitOpen: twelveDataDisabled || twelveDataQuotaExhausted || Date.now() < twelveDataOpenUntil,
    twelveDataDisabled,
    twelveDataQuotaExhausted,
    finnhubCircuitOpen: Date.now() < finnhubOpenUntil,
