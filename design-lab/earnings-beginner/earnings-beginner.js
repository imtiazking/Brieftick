/**
 * Earnings · beginner-first preview (design-lab)
 * @module design-lab/earnings-beginner/earnings-beginner
 */
...
/** Set true only when production earnings API is wired; never imply live data while false. */
const EARN_USES_LIVE_DATA = false;

const earnState = { filter: "all", search: "", events: [], selectedId: null, source: "demo" };

function dataBadgeLabel() {
  return EARN_USES_LIVE_DATA ? "Live data" : "Demo data";
}

function dataDisclosureText() {
  return EARN_USES_LIVE_DATA
    ? "Earnings data updates from live market feeds."
    : "Earnings data shown here is sample preview data until live earnings integration is connected.";
}
    ...
    (weekEarnings[dayIndex] || []).forEach(([sym, session]) => {
      events.push(
        makeEarnEvent({
          ...
          dayIndex,
          epsExp: sym === "NVDA" ? "$4.87" : sym === "MSFT" ? "$3.42" : "—",
          revExp: sym === "NVDA" ? "$31.1B" : sym === "MSFT" ? "$68.4B" : "—",
        })
      );
    });
  });
...
function ensureHeroShell() {
  ...
  el.innerHTML = `
    ...
    <div class="earn-v2-detail-actions">
      ...
      <button type="button" class="earn-v2-btn earn-v2-btn--disabled" id="earnHeroLogic" disabled>Ask Logic — Coming Soon</button>
    </div>`;
  return el;
}
...
function renderCalendar() {
  ...
  grid.innerHTML = days
    .map((d, i) => {
      const earns = events.filter((e) => e.dayIndex === i);
      return `
...
function openDeepDive(sym) {
  ...
  window.open(`/?sym=${s}&tab=overview`, "_blank");
}
